...
.github/workflows/        Build + deploy estático para o GitHub Pages
```
...
3. Deploy — veja a opção 3a (GitHub Pages) ou 3b (Vercel/Netlify) abaixo.

#### 3a. Deploy no GitHub Pages

O app é gerado como export estático (`output: "export"` em
`next.config.ts`) porque o GitHub Pages só serve arquivos estáticos — não há
servidor Next.js rodando. Por isso o link do cliente é
`/aprovar/?token=...` (query string) em vez de `/aprovar/{token}` (rota
dinâmica não dá pra pré-gerar para todo token possível).

1. Em **Settings > Pages** do repositório, em "Build and deployment",
   selecione **Source: GitHub Actions**.
2. Em **Settings > Secrets and variables > Actions > Variables**, crie a
   variável `NEXT_PUBLIC_SUPABASE_FUNCTIONS_URL` com a URL das Edge
   Functions (é pública — só o endpoint, não uma chave secreta).
3. Dê push/merge na branch `main` — o workflow
   `.github/workflows/deploy-pages.yml` builda e publica automaticamente em
   `https://<usuário>.github.io/<repo>/`.
4. O link a enviar para cada cliente fica
   `https://<usuário>.github.io/<repo>/aprovar/?token={access_token}`.

#### 3b. Deploy no Vercel/Netlify

Se preferir rodar como app Next.js normal (sem as limitações do GitHub
Pages), remova `output: "export"` de `next.config.ts`, conecte o repo no
Vercel/Netlify e configure a mesma env var `NEXT_PUBLIC_SUPABASE_FUNCTIONS_URL`
no projeto. Nesse caso a rota `/aprovar/{token}` também funcionaria, mas o
código atual já usa `/aprovar/?token=...` nos dois casos para manter uma
única versão do frontend.
